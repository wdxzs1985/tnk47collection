...
public class PrefectureBattleListHandler extends AbstractBattleHandler {
	...
	protected String handleIt() {
		...
		final JSONObject prefectureBattleSystemDto = data
				.getJSONObject("prefectureBattleSystemDto");
		final String prefectureBattleSystemStatus = prefectureBattleSystemDto
				.getString("prefectureBattleSystemStatus");
		if (StringUtils.equals(prefectureBattleSystemStatus, "ACTIVE")) {
			final JSONArray prefectureBattleOutlines = data
					.getJSONArray("prefectureBattleOutlines");
			if (prefectureBattleOutlines.size() > 0) {
				final JSONObject battle = this
						.filterInviteBattle(prefectureBattleOutlines);
				final String prefectureBattleId = battle
						.getString("prefectureBattleId");
				session.put("prefectureBattleId", prefectureBattleId);
				if (this.log.isInfoEnabled()) {
					final String otherPrefectureName = battle
							.getString("otherPrefectureName");
					this.log.info(String.format("jinru和%s的合战",
							otherPrefectureName));
				}
			...
			final JSONArray prefectureBattleUsers = data
					.getJSONArray("prefectureBattleUsers");
			if (prefectureBattleUsers.size() > 0) {
				final JSONObject user = this
						.filterLowPowerUser(prefectureBattleUsers);
	...
	private JSONObject filterInviteBattle(final JSONArray outlines) {
		for (int i = 0; i < outlines.size(); i++) {
			...
			if (battle.getBoolean("isInvite")) {
				if (this.log.isInfoEnabled()) {
					this.log.info("shou dao jiu yuan qing qiu");
				}
				return battle;
			}
		}
