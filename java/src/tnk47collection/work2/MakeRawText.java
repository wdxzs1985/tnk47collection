...
public class MakeRawText implements Runnable {
	...
	public void run() {
		...
		try {
			final List<String> inputLines = FileUtils
			...
			for (final String line : inputLines) {
				...
				} else if ((matcher = MakeRawText.TAG_REGION.matcher(found))
						.find()) {
					outputLines.add(matcher.group(1));
				} else if ((matcher = MakeRawText.TAG_NAME.matcher(found))
						.find()) {
					outputLines.add(matcher.group(1));
