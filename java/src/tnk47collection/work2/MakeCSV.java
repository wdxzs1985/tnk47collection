...
public class MakeCSV implements Runnable {
	...
	public void run() {
		System.out.printf("%s(%d) start\n", this.getClass().getSimpleName(),
				this.number);
		String input = String.format("data2/step2/%d.txt", this.number);
		String output = String.format("data2/step3/%d.csv", this.number);
		...
		try {
			...
			for (int i = 0; i < inputLines.size(); i++) {
				...
				Matcher imgMatcher = PATTERN_IMG.matcher(line);
				if (imgMatcher.find()) {
